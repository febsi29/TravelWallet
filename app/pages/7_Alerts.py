...
with st.expander("偵測參數"):
    ...
    with c1:
        z_th = st.slider("Z-Score 門檻", 1.0, 3.0, 2.0, 0.1)
    with c2:
        iqr_m = st.slider("IQR 倍數", 1.0, 3.0, 1.5, 0.1)
    with c3:
        if_c = st.slider("隔離森林污染率", 0.05, 0.3, 0.1, 0.01)
...
if st.button("執行偵測", type="primary", use_container_width=True):
    ...
    if anomalies:
        ...
        for a in anomalies:
            ...
            with col_info:
                st.error(
                    f"#{a['txn_id']} | NT${a['amount_twd']:,.0f} | {a['category']} | {a['description']} | 標記 {a['flags']}/3"
                )
            with col_btn:
