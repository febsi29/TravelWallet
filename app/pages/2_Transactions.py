...
# 白名單映射：排序欄位與方向均為固定常數，不接受使用者原始輸入
_ORDER_COL = {
    "時間(新到舊)": "t.txn_datetime",
    "時間(舊到新)": "t.txn_datetime",
    "金額(高到低)": "t.amount_twd",
    "金額(低到高)": "t.amount_twd",
}
_ORDER_DIR = {
...
with sqlite3.connect(DB_PATH) as conn:
    ...
    order_col = _ORDER_COL[sort_by]
    order_dir = _ORDER_DIR[sort_by]
    ...
    if search:
        base_query += " AND t.description LIKE ?"
        params.append(f"%{search}%")
    # ORDER BY 使用白名單常數字串拼接，不含任何使用者輸入
