...
for code, info in COMMON_CURRENCIES.items():
    ...
    rate_data.append(
        {
            "貨幣": f"{info['symbol']} {code}",
            "名稱": info["name"],
            "國家": info["country"],
            "1 TWD =": f"{rate:.4f} {code}",
            "1 單位 =": f"NT${twd_per:,.2f}",
        }
    )
...
with c2:
    st.markdown(
        "<br><br><p style='text-align:center; font-size:2rem'>-></p>", unsafe_allow_html=True
    )
with c3:
    ...
    if st.button("換算", type="primary", use_container_width=True):
        ...
        to_str = (
            cm.format_amount(result["amount"], to_curr)
            if to_curr != "TWD"
            else f"NT${result['amount']:,.0f}"
        )
...
with st.expander("新增提醒", expanded=False):
    ...
    with col3:
        direction = st.selectbox(
            "方向", ["above", "below"], format_func=lambda x: "高於" if x == "above" else "低於"
        )
    with col4:
