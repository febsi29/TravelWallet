...
with tab1:
    ...
    with sqlite3.connect(DB_PATH) as _c:
        nm = _c.execute("SELECT COUNT(*) FROM trip_members WHERE trip_id=?", (trip_id,)).fetchone()[
            0
        ]
        tot = _c.execute(
            "SELECT COALESCE(SUM(amount_twd),0) FROM transactions WHERE trip_id=?", (trip_id,)
        ).fetchone()[0]
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                name="本團每人平均",
                x=["行程總計", "每日平均"],
                y=[p["per_person_total"], p["per_person_daily"]],
                marker_color="#2563EB",
            ),
            go.Bar(
                name="全國每人平均",
                x=["行程總計", "每日平均"],
                y=[n["avg_total"], n["avg_daily"]],
                marker_color="#93C5FD",
            ),
        ]
    )
...
with tab3:
    ...
    if daily:
        ...
        fig.update_layout(
            yaxis=dict(title="每日消費 (NT$)"),
            yaxis2=dict(title="累計消費 (NT$)", overlaying="y", side="right"),
            height=400,
        )
        st.plotly_chart(fig, use_container_width=True)
...
with col3:
    try:
        ...
        pvn_rows = [
            {
                "項目": "每人總花費 (NT$)",
                "個人": pvn["personal"]["per_person_total"],
                "全國平均": pvn["national"]["avg_total"],
            },
            {
                "項目": "每人每日花費 (NT$)",
                "個人": pvn["personal"]["per_person_daily"],
                "全國平均": pvn["national"]["avg_daily"],
            },
