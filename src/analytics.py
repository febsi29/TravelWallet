...
class Analytics:
    ...
    def personal_vs_national(self, trip_id: int) -> dict:
        ...
        diff_pct = (
            round((per_person_total / national_total - 1) * 100, 1) if national_total > 0 else 0
        )
        ...
        return {
            ...
            "national": {
                "year": trip_year,
                "avg_total": national_total,
                "avg_nights": national_nights,
                "avg_daily": national_daily,
            },
            "comparison": {
    ...
    def split_behavior(self, trip_id: int) -> dict:
        ...
        with self._db() as (conn, cursor):
            ...
            payer_ranking = [
                {"name": r[0], "times": r[1], "total_twd": r[2]} for r in cursor.fetchall()
            ]
    ...
    def payment_analysis(self, trip_id: int) -> list:
        ...
        return [
            {
                "method": labels.get(r[0], r[0]),
                "count": r[1],
                "total_twd": r[2],
                "percentage": round(r[2] / grand_total * 100, 1),
            }
            for r in rows
        ]
...
if __name__ == "__main__":
    ...
    print(f"  {'-' * 50}")
    print(
        f"  {'Per Person':15s} NT${p['per_person_total']:>8,} NT${n['avg_total']:>8,.0f} {c['diff_total']:>+8,.0f}"
    )
    print(
        f"  {'Daily':15s} NT${p['per_person_daily']:>8,} NT${n['avg_daily']:>8,} {c['diff_daily']:>+8,}"
    )
