...
8 位使用者，5 趟不同組合的旅行：
  Trip 1: 東京自由行    - 示範使用者 A、B、C、D（4人）
  Trip 2: 首爾追星之旅  - 示範使用者 E、F、G、H（4人）
  Trip 3: 清邁探索之旅  - 示範使用者 A、C、E、F（4人）
  Trip 4: 沖繩海島行    - 示範使用者 B、D、G、H（4人）
...
def calculate_settlements(cursor, trip_id, user_ids, rate):
    ...
    while i < len(creditors) and j < len(debtors):
        ...
        amt = min(credit, debt)
        transfers.append(
            {"from": did, "to": cid, "amount": round(amt), "amount_twd": round(amt * rate)}
        )
...
def seed_all():
    ...
    for uid in user_ids:
        for cur, bal in [
            ("TWD", round(random.uniform(5000, 30000), 0)),
            ("JPY", round(random.uniform(5000, 50000), 0)),
        ]:
            c.execute(
                "INSERT INTO wallets (user_id, currency_code, balance, locked_balance) VALUES (?, ?, ?, 0)",
                (uid, cur, bal),
            )
    ...
    for trip_def, txns, customs in ALL_TRIPS:
        ...
        c.execute(
            ...
            (
                creator_id,
                trip_def["trip_name"],
                trip_def["destination"],
                trip_def["currency_code"],
                trip_def["start_date"],
                trip_def["end_date"],
                trip_def["total_budget"],
                trip_def["status"],
            ),
        )
        trip_id = c.lastrowid
        ...
        for i, txn in enumerate(txns):
            ...
            dt_str = (start_dt + timedelta(days=day, hours=hour, minutes=minute)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        ...
        if trip_def["status"] == "completed":
            settled_at = (start_dt + timedelta(days=len(set(t[0] for t in txns)) + 1)).strftime(
                "%Y-%m-%d 12:00:00"
            )
